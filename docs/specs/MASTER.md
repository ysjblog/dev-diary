...
| Change | 目的 | 狀態 |
|---|---|---|
| [`add-codex-desktop-auto-resume`](../../openspec/changes/add-codex-desktop-auto-resume/proposal.md) | 整合 macOS Codex Desktop 多任務 deep-link 額度恢復續跑；thread ID 精確定位、背景只送固定「繼續」 | v8 已實作並 commit（本機）；3.1 O3 收門審查、3.4 獨立黑箱 QA、6.4 可靠無人啟動證據仍待完成，完成後才 archive |
