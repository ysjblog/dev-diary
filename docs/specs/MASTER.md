...
| SQLite schema / 儲存 | ✅ schema + deterministic seed 完成（projects/sessions/token_usage/daily_logs/comments/kanban/project_docs/app_settings/log_file_scan_cache）；Core startup 預設使用 `~/Library/Application Support/DevDiary/DevDiary.sqlite`；`custom_agents` 持久化於 app settings；`log_file_scan_cache` 以 `(agent_name, file_path)` 持久化逐檔解析結果並依 `mtime_ms` 失效；`kanban_cards.status_locked_by_user` 會保護使用者手動移動過的卡片狀態 |
| Dashboard 聚合（§10） | ✅ 單一 snapshot 驅動 metric/mix/trend/project concentration；range-based ranking 與不跟 range 連動的 latest-26-week heatmap window 已接；`近24小時` trend 已改用 `sessions.start_time` 依小時分桶（固定 24 點），其餘 range 仍依日期分桶不變；Core/root tests + runtime smoke |
...
| Manual Scan / Project Rescan | ✅ Core endpoint + React button wiring 已完成；manual / project / background scan 統一記錄 operation 與最新完成時間，下一輪 background due 以最後完成者重算；runtime 預設使用 persistent SQLite + CLI log provider；app-facing Settings 固定真實 CLI logs 且不做 fake fallback；底層 deterministic mock provider 只供測試或明確 provider injection；global scan 可先從 configured roots upsert project folders |
| CLI log parser（Claude/Codex/Antigravity） | ✅ Claude Code / Codex CLI JSONL parser provider 已接入 scan contract 並成為 default scan provider；Antigravity CLI glog / transcript metadata parser 已可建立低信心 sessions；逐檔 SQLite mtime cache 已解除原本的歷史檔案數／天數視窗限制，實機掃描目前 project 得到 Codex 110、Claude 14、Antigravity 5 sessions，token 欄位仍可能為 0 並清楚保留 experimental/低信心語意 |
| AI Diary Agent + fallback | ✅ Workspace summary regenerate 已接 Core Diary Agent contract；Claude Code、Codex CLI（固定 read-only `codex exec` argv，含 `--skip-git-repo-check`）與 enabled `antigravity-cli` 都可產生 Markdown draft，Claude resolver 不會把 Claude.app desktop GUI launcher 當成 print CLI；safe child env 保留必要的 non-secret runtime identity；default diary agent 也可指向 configured local Ollama custom agent，透過 loopback `/api/generate` 使用 selected model（例如 `qwen3.6:27b`）產生 Markdown draft；失敗/逾時/disabled 會寫 deterministic fallback draft；Core in-app daily scheduler 已可每日產生白話 global daily highlight、刷新 project AI drafts、同步 Kanban synthesis，並在 Run now 回傳分層 preflight 狀態 |
| UI 接 Core API（取代 mock） | ✅ Dashboard 全頁 + sidebar 24h 已接，background scan running state 會驅動 sidebar spinner 與 5 秒 polling；Dashboard AI Global Summary List 改讀 Core `daily_highlights` 並移除固定 mock；Projects Workspace 讀取路徑與主要寫入路徑已接（comments、Kanban status、summary override / AI draft、daily diary entry）；Docs preview 已改大型 rich Markdown modal，Docs 依 source mtime 排序、動態 path 分組且可依檔名/內容搜尋；Settings UI 已接 `GET/PATCH /api/settings`、Local HTTP API runtime status/stale warning、daily scheduler 設定/Run now、Markdown daily export、redacted structured backup、project docs filename allowlist、AI prompt overrides；CLI Agents 頁已接 settings enabled toggle、canonical executable / activity-log source controls、Core safe detection、custom agent safe probe/persist、default diary agent selector、agent model/reasoning preferences；first-launch onboarding 已接 Core settings/detection/scan |
| Tauri shell + 打包 | ✅ Tauri shell 可啟動 loopback Core 並在 quit 後清理 process；正式 `npm run package:mac` 使用 Tauri `--no-sign` 加 staging DMG 的最終完整 ad-hoc seal，含 Applications drag-install 背景、bundled official Node 22.23.1、quarantine 診斷與 GitHub workflow；background alias 暫存 mount path regression 已修正並通過 fresh Finder QA；LaunchAgent 不會改寫 bundle；使用者首次開啟需手動允許；Tauri 前端已不再寫死 Core API origin，改由 `resolve_core_api_origin` command 讀 runtime manifest 取得 Core 實際 bind 的 port（4317 被佔用時的 fallback port），與既有 dev-mode Vite proxy 的 manifest discovery 機制一致，manifest 缺失/過期時 fallback 回 `127.0.0.1:4317` |
...
- `deltas/dashboard-24h-hourly-trend-delta.md` — Dashboard 近24小時趨勢圖改用小時分桶，取代原本退化成單點水平線的日期分桶（merged, branch fix/dashboard-24h-hourly-trend）

- `deltas/log-scan-unlimited-history-delta.md` — CLI 日誌逐檔 mtime cache，支援無上限歷史紀錄掃描（merged, branch fix/log-scan-unlimited-history；實機 129 sessions、無 warnings）

- `deltas/agent-runtime-reliability-ui-delta.md` — Claude Diary Agent print-mode adapter、source-path UI row controls、comment draft/SQLite busy hardening、background scan state 與 LaunchAgent migration/startup contention fix（implemented / verified）

- `deltas/scan-diary-docs-usability-delta.md` — 手動／背景 scan 單一完成時間基準、Claude/Codex diary adapter、CLI executable UI、Docs source-mtime/path group/search（implemented / verified，branch `codex/agent-source-path-settings-public`）

- `deltas/agent-source-path-settings-delta.md` — canonical agent 的 CLI executable 與 activity log roots 分離顯示、自動偵測與手動設定（implemented / verified，branch `codex/fix-dmg-onboarding-cli-discovery`）
...
- `deltas/macos-signed-release-distribution-delta.md` — 2026-07-11 follow-up：移除會把 Finder background alias 綁死 build 暫存 mount path 的 metadata rewrite，verifier 與 fresh Finder QA passed
