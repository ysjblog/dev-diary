...
- Local development preset scripts：`npm run start:local` / `npm run dev:local` 明確帶入本機 development project roots：
  - `~/Projects`
...
- Explicit `DEVDIARY_DB=:memory:` 仍保留 dev/test local roots preset 與 seed data 行為。
...
- [x] `npm run start:local` / `npm run dev:local` 可用 single command 帶入本機 development roots。
