...
**期待輸出**：語法與 plist 均有效；plist `ProgramArguments` 指向固定 shell + launcher + `run`；在 `$HOME` 位於 noowners external-volume 時，實體 plist 放在內建磁碟專案 `.launchagents/`，`~/Library/LaunchAgents` 只放 symlink。
