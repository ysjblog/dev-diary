...
- [ ] Operation order invariants: 公開 snapshot 以 `origin/main` 為 base，避免推送本機 private history。
...
- [ ] State/history/retry/refresh behavior: remote `main` 不被 force-push，公開 snapshot 以 fast-forward 推送。
- [ ] Externally observable result, not only implementation detail: GitHub `main` 可快轉到已清理的 snapshot。
