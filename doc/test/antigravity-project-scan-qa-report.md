...
- [PASS] Global scan discovers configured root folders. Expected `/api/scan?range=all` to discover projects under the configured example roots; actual project count 20 across the example project roots.
...
- [PASS] Antigravity CLI records are visible through project detail API. Expected at least one session with `source_log_ref` beginning `antigravity-cli://`; actual 50 sessions, sample project root under the configured example roots.
