...
Increase the claim/action lease from 30 to 45 seconds to cover queue 15s + cleanup 4s + open 5s + cleanup 4s with margin. The diagnostics observer adds at most a 2-second baseline probe before dispatch and a 4-second snapshot probe after a failed-source dispatch inside the same lease; the worst case (about 34 seconds) stays within 45 seconds. Keep startup deadline 15s. Schema, API state enum and completed-evidence semantics are unchanged. Legacy resumed rows are not reopened on upgrade. Existing pause/mutation exclusion remains for the complete queue+open action.
