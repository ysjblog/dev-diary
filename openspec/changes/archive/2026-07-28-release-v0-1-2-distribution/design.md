...
At planning time `main` was published at `92c7708`, while the only current GitHub Release was `v0.1.1` at `7330756`. At publication, `origin/main` and annotated `v0.1.2` both identified immutable release candidate `8c71412`; `v0.1.1` remains unchanged. Later closeout documentation commits record evidence only and are intentionally not part of the released DMG or tag.
...
- Release source of truth: immutable `v0.1.2` / `8c71412` plus the four version declarations. Current closeout documentation on `main` records evidence only and does not retarget the release.
